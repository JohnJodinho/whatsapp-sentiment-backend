...
# We need this to embed the user's question before searching
...
# --- 2. "Second-Hop" Context Fetchers ---
# These functions perform the fast, bulk lookups we discussed
...
async def _fetch_message_context(db_session_factory: Any, ids: List[int]) -> Dict[int, str]:
    ...
    """
    async with db_session_factory() as db: # <-- CREATE OWN SESSION
        stmt = (
            select(models.Message)
            .options(selectinload(models.Message.participant)) # Eager load participant
...
async def _fetch_time_segment_context(db_session_factory: Any, ids: List[int]) -> Dict[int, str]:
    """
    Bulk fetches context for 'segments_time' sources.
    """
    async with db_session_factory() as db:
        stmt = select(models.TimeSegment).where(models.TimeSegment.id.in_(ids))
        result = await db.execute(stmt)
    ...
    for s in segments:
        ...
        context_map[s.id] = f"Summary of chat from {start_str} to {end_str}:\n{s.summary}"
    return context_map
...
class SQLAlchemyVectorRetriever(BaseRetriever):
    ...
    async def _aget_relevant_documents(
        ...
        *, 
        ...
        if source_ids_map["segments_time"]:
            tasks.append(_fetch_time_segment_context(self.db_session_factory, source_ids_map["segments_time"]))
